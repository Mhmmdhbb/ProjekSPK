...
@section('content')
<div class="max-w-3xl">
    <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Penilaian</h1>
    <p class="text-gray-600 mb-6">{{ $alternatif->kode_alternatif }} - {{ $alternatif->nama_alternatif }}</p>
    ...
    <form action="{{ route('penilaian.update', $alternatif->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        @foreach ($kriterias as $kriteria)
            <div class="mb-6 p-4 bg-gray-50 rounded-lg border">
                ...
                @php $currentValue = $existingPenilaian[$kriteria->id] ?? null; @endphp
                ...
                @if($kriteria->subKriteria->count() > 0)
                    <select name="nilai[{{ $kriteria->id }}]" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-green-600" required>
                        ...
                        @foreach ($kriteria->subKriteria as $sub)
                            <option value="{{ $sub->nilai }}" {{ (old('nilai.' . $kriteria->id, $currentValue) == $sub->nilai) ? 'selected' : '' }}>
                                {{ $sub->nama_sub }} (Nilai: {{ $sub->nilai }})
                            </option>
                        @endforeach
                    </select>
                @else
                    <input type="number" name="nilai[{{ $kriteria->id }}]" value="{{ old('nilai.' . $kriteria->id, $currentValue) }}" 
